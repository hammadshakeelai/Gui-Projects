...
static constexpr int spawnInterval = 60; // frames between pillars
...
private:
    int x_, y_;
    float velocity_;
    static constexpr int size_ = 30;
    static constexpr float gravity_ = 0.4f; // Gravity effect
};
...
private:
    ...
    static constexpr int gap_ = 150;
    static constexpr int speed_ = 6; // Speed of the pillars
};
...
public:
    ...
    void update() {
        ...
        if (!pillars_.empty() && pillars_.front().isOffScreen()) {
            ...
            pillarsCrossed_++; // Increment the counter for crossed pillars
